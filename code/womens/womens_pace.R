...
for (i in 2014:2022)
{
  ...
  schedule <- schedule %>%
    bind_rows(load_wbb_schedule(i) %>%
                select(id, home_location, away_location, neutral_site, conference_competition,
                       status_type_detail) %>%
                mutate(id = as.integer(id)))
}

home <- schedule %>%
  select(id, team = home_location, neutral_site, conference_competition, game_result = status_type_detail) %>%
  mutate(home = T)

away <- schedule %>%
  select(id, team = away_location, neutral_site, conference_competition, game_result = status_type_detail) %>%
  mutate(home = F)
...
write_csv(data, "../../data/womens/womens_team_pace.csv")
