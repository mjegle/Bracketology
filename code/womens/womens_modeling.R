...
####### Try random forest ##########
stats_seeds <- read_csv("../data/womens/womens_stats_seeds.csv")
...
stats_seeds <- stats_seeds %>%
  ...
  mutate(conference = as.factor(conference),
         ...
         Seed = as.factor(Seed),
         qualified = ifelse(Seed == "DNQ", 0, 1))

train <- stats_seeds %>%
  filter(year < 2022) %>%
  select(qualified, team, Seed, num_game, o_reb_rate:opp_steal_rate, ftf, ftf_opp, avg_bids)
...
# Binary classifier
tictoc::tic()
qualified_rf <- randomForest::randomForest(qualified ~ .-team-Seed,
                                              data = train)
tictoc::toc()
...
seed_rf_reduced <- randomForest::randomForest(Seed ~ net_rating + conference + off_rating + o_reb_rate + def_rating + sos_off +
                                                ...
                                                ftf + ftf_opp + num_game + avg_bids,
                                              data = train)
tictoc::toc()
importance <- randomForest::importance(seed_rf_reduced) %>%
  as.data.frame()

pred <- predict(seed_rf_reduced, test, type = "prob")

pred_seed <- predict(seed_rf_reduced, newdata = test)
table(pred_seed, test$Seed)
mean(pred_seed == test$Seed)
...
##### Why the fuck is princeton always so high???? #####
test %>%
  filter(team == "Princeton") %>%
  tail(1) -> princeton
